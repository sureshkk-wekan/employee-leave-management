...
- **Persistence:** Use a single database (e.g. SQLite for small scale, PostgreSQL for production); schema for users, leave types, requests, approvals, and balance snapshots.
...
- **Policy as Data:** Leave types, entitlements, and approval rules are stored as data (e.g. in DB or config) rather than hardcoded, to allow changes without code deploys.
...
- **Authentication:** All leave and approval endpoints require authenticated users; use sessions or JWT as chosen by the stack; never expose approval actions to unauthenticated or wrong-role users.
...
- **Secrets & Config:** Store secrets (DB URL, session secret, API keys) in environment or `.env`; never commit secrets to the repository.
